...
def convert_wav_to_fft(file_name, points_per_fft, max_range_bits=16, \
                       mod_freqs=0):
    ...
    Args:
        ...
        max_range_bits: The number of bits used to represent the FFT magnitudes
    ...
    while (i < num_frames):
        ...
        # get the min/max
        min_freq = numpy.amin(freqs)
        max_freq = numpy.amax(freqs)

        # normalize between the given range
        # between 0 and 2**max_range_bits
        freqs = (freqs - min_freq) * (2**(max_range_bits) - 1) / (max_freq - min_freq)
        ...
        if (mod_freqs): # don't do fft modification to match output of Xilinx FFT block
...
if __name__ == "__main__":
    ...
    #        1024)
    convert_wav_to_fft('sample_wav_files/a.wav', 1024, max_range_bits=16)
